...
def train(config):
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        tb_logger.update(epoch, {'train_loss': sum(running_loss) / len(running_loss), 'test_loss': {sum(test_loss) / len(test_loss)},'learning_rate': learning_rate})

        log = f'{epoch}, train_loss: {sum(running_loss) / len(running_loss):.3f}, test_loss: {sum(test_loss) / len(test_loss):3f}, epoch time {(time.time() - epoch_start):1f} secs'
        logger.info(log)
